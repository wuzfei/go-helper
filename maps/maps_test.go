...
var m3 = map[string]t1{"a": t1("a"), "b": t1("b"), "c": t1("c")}
...
func TestToStringFunc(t *testing.T) {
	...
	s1 := "a=a&b=b&c=c"
	if s != s1 {
		t.Errorf("ToQueryString(%v) = %v, want %v", m3, s, s1)
	}
	s2 := ToStringFunc(m3, ";", func(k string, v t1) string {
		return fmt.Sprintf("%s:%s", k, v)
	})
	s3 := "a:a;b:b;c:c"
	if s2 != s3 {
		t.Errorf("ToQueryString(%v) = %v, want %v", m3, s2, s3)
	}
}
